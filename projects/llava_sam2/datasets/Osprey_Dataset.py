...
class OspreyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(self.image_folder, list):
            barecat_dir = Path(self.image_folder[0]).parent
            with barecat.Barecat(barecat_dir / "coco.barecat", readonly=True) as bc:
                for image_folder in self.image_folder:
                    image_path = os.path.join(image_folder, image_file)
                    image_filepath = str(Path(image_path).relative_to(barecat_dir))
                    if bc.exists(image_filepath):
                        with bc.open(image_filepath, 'rb') as f:
