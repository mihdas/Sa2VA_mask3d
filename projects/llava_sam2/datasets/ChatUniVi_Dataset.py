...
def _get_rawvideo_dec(video_path, select_frames=5):
    if "barecat" in video_path:
        video_path = Path(video_path)
        barecat_dir = video_path.parent
        with barecat.Barecat(barecat_dir / "activity_videos.barecat", readonly=True) as bc:
            video_path = "Activity_Videos/" + str(video_path.name)
            if bc.exists(video_path):
                with bc.open(video_path, 'rb') as f:
                    vreader = VideoReader(f, ctx=cpu(0))
            elif bc.exists(video_path.replace('mkv', 'mp4')):
                with bc.open(video_path.replace('mkv', 'mp4'), 'rb') as f:
                    vreader = VideoReader(f, ctx=cpu(0))
            else:
                print(video_path)
...
class VideoChatUniViDataset(Dataset):
    ...
    def dataset_map_fn(self, data_dict, select_k=5):
        ...
        actual_num_frames = len(images)  # Get actual number of frames returned
        if self.use_fast:
        ...
        # prepare text - use actual number of frames, not requested
        if self.use_fast:
            text_dict = self.prepare_text(
                actual_num_frames, conversation, num_image_tokens=self.patch_token,
        ...
        ret = {'images': images, 'conversation': text_dict['conversation'], 'fast_images': fast_images, 'actual_num_frames': actual_num_frames}
        return ret
