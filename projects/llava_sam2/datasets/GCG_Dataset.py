...
class GCGDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with barecat.Barecat(Path(self.image_folder) / barecat_filename, readonly=True) as bc:
            if "flickr" in Path(self.image_folder).name:
                image_file = "flickr30k-images/" + image_file
            elif "GranDf_HA_images" in Path(self.image_folder).name:
                image_file = "GranDf_HA_images/train/" + image_file
            elif "COCO" in image_file:
                _, coco_folder, _ = image_file.split("_")
                image_file = str(Path(coco_folder) / image_file)
            with bc.open(image_file, 'rb') as f:
        ...
        new_masks = list()
        for mask_idx in range(len(data_dict["masks"])):
            mask = dynamic_preprocess_mask(
                data_dict["masks"][mask_idx].numpy(),
                self.min_dynamic_patch,
                self.max_dynamic_patch,
                self.image_size,
                self.use_thumbnail)
            mask = torch.stack([torch.from_numpy(m) for m in mask])
            new_masks.append(mask)
        data_dict["masks"] = torch.stack(new_masks).clone()
