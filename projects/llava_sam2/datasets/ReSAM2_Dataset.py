...
from xtuner.dataset.huggingface import build_origin_dataset, process_hf_dataset
...
class VideoSAM2Dataset(Dataset):
    ...
    def __init__(
        ...
        print(
            "Video res dataset (ref-sam2), include {} items.".format(
                len(self.video_ids)
            )
        )
    ...
    def dataset_map_fn(self, objects_expression_infos, n_frames, n_fast_frames=0):
        ...
        if self.mode == "long":
            expressions = [
                object_info["formated"] for object_info in objects_expression_infos
            ]
            self.cur_mode = self.mode
        elif self.mode == "short":
            expressions = [
                object_info["short_caps"][
                    random.randint(0, len(object_info["short_caps"]) - 1)
                ]
                for object_info in objects_expression_infos
            ]
            self.cur_mode = self.mode
        else:
    ...
    def prepare_text(
        self, n_frames, expressions, num_image_tokens=256, n_fast_frames=0
    ):
        if self.use_fast:
    ...
    def __getitem__(self, index):
        ...
        video_path = Path(expression_dict["video_path"])
        anno_path = Path(expression_dict["anno_path"])

        # Remove the first folder
        video_path = Path(*video_path.parts[1:])
        anno_path = Path(*anno_path.parts[1:])

        # Join with the new base folder
        video_path = Path(self.sam2_folder) / video_path
        anno_path = Path(self.sam2_folder) / anno_path
        ...
        if self.use_fast:
            ...
            sampled_fast_frame_idxs = [
                min(int(i * fast_interval), len(video_frames) - 1)
        ...
        if video_frames is not None:
            ...
            index += 1
            print(f"{video_path} Cannot be opened correctly! Using the next video at {index}")
        ...
        if n_objects > self.select_number:
            ...
            selected_indexes = np.random.choice(
                n_objects, self.select_number, replace=True
            )

        selected_object_ids = [object_ids[_idx] for _idx in selected_indexes]
        objects_expression_infos = [
            expression_dict["objects"][_idx] for _idx in selected_object_ids
        ]
        _masklents = []
        ...
        (
            ...
            vid_len=n_frames,
            num_frames=self.sampled_frames,
            tarvis_num_frames=self.tarvis_sampled_frames,
            train_mode=True,
        )
        ...
        tarvis_video_frames = [
            video_frames[_idx] for _idx in tarvis_selected_frame_indexes
        ]
        video_frames = [video_frames[_idx] for _idx in selected_frame_indexes]
        masklents = [masklents[_idx] for _idx in tarvis_selected_frame_indexes]

        data_dict = self.dataset_map_fn(
            objects_expression_infos,
            len(video_frames),
            n_fast_frames=self.n_fast_images,
        )
        ...
        for frame in tarvis_video_frames:
            frame = frame[:, :, ::-1]
            frame_image = Image.fromarray(frame).convert("RGB")
...
def add_mask2images(frames, masklets):
    ...
    for i_frames, (frame, masks) in enumerate(zip(frames, masklets)):
        ...
        n_obj = masks.shape[-1]
        for i_obj in range(n_obj):
            show_videos[i_obj].append(
                draw_mask(copy.deepcopy(frame), masks[:, :, i_obj])
            )
    return show_videos
