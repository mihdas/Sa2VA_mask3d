...
def video_lisa_collate_fn(instances: Sequence[Dict],
    ...
    has_3d_features = any(inst.get('features_3d') is not None for inst in instances)
    ...
    has_obj_ids = any(inst.get('obj_ids') is not None for inst in instances)
    has_scene_id = any(inst.get('scene_id') is not None for inst in instances)
    has_gt = any(inst.get('gt') is not None for inst in instances)
    has_projected = any(inst.get('projected') is not None for inst in instances)
...
#    print(instances[0].keys())
    ...
    if has_3d_features:
        data_dict['features_3d'] = instances[0]['features_3d']
    ...
    if has_obj_ids:
        data_dict['obj_ids']=instances[0]['obj_ids']
    if has_mask:
        data_dict['masks'] = object_masks
    if has_scene_id:
        data_dict['scene_id']=instances[0]['scene_id']

    if has_projected:
        data_dict['projected']=instances[0]['projected']

    if has_gt:
        data_dict['gt']=instances[0]['gt']
    if has_bboxes:
