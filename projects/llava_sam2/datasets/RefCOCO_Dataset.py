...
class ReferSegmDataset(RefCocoDataset):
    ...
    def _parse_annotations(self, ann_info):
        ...
        barecat_dir = Path(self.data_prefix["img_path"]).parent
        with barecat.Barecat(barecat_dir / "coco.barecat", readonly=True) as bc:
            with bc.open(Path(image_path).relative_to(barecat_dir), 'rb') as f:
        ...
        for idx in index:
            ...
            binary_mask = dynamic_preprocess_mask(binary_mask, self.min_dynamic_patch, self.max_dynamic_patch, self.image_size, self.use_thumbnail)
    ...
    def prepare_data(self, index):
        ...
        if data_dict.get('image', None) is not None:
            ...
            try:
                barecat_dir = Path(self.data_prefix["img_path"]).parent
                with barecat.Barecat(barecat_dir / "coco.barecat", readonly=True) as bc:
                    with bc.open(Path(image_file).relative_to(barecat_dir), 'rb') as f:
                        image = Image.open(f).convert('RGB')
