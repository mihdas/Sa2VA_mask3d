...
class VideoReVOSDataset(Dataset):
    ...
    def __init__(
        ...
        self.tarvis_sampled_frames = tarvis_sampled_frames
        ...
        if offline_processed_text_folder is not None:
            ...
            vid2metaid, metas, mask_dict = self.json_file_preprocess(
                expression_file, mask_file
            )
            self.vid2metaid = vid2metaid
            self.videos = list(self.vid2metaid.keys())
            self.mask_dict = mask_dict
            json_data = DatasetDict({"train": HFDataset.from_list(metas)})
            if self.lazy:
        ...
        for vid_name in expression_datas:
            ...
            for exp_id in exp_id_list:
                ...
                if "obj_id" in exp_dict.keys():
                    ...
                    meta["obj_id"] = [
                        0,
                    ]  # Ref-Youtube-VOS only has one object per expression
                meta["anno_id"] = [
                    str(anno_count),
                ]
                anno_count += 1
            ...
            if len(object_masks) == 0:
                if len(ret_masks) != 0:
                    ...
                    _object_masks = np.zeros(
                        (self.tarvis_sampled_frames, image_size[0], image_size[1]),
                        dtype=np.uint8,
                    )
            else:
                _object_masks = []
    ...
    def dataset_map_fn(self, data_dict, select_k=8, select_k_tarvis=16):
        images = []
        tarvis_images = []
        ...
        #         selected_start_frame = np.random.choice(
        #             len_frames - select_k, 1, replace=False
        #         )
        #         selected_frame_indexes = [
        #             selected_start_frame[0] + _i for _i in range(select_k)
        #         ]
        #     else:
        #         selected_frame_indexes = np.random.choice(
        #             len_frames, select_k, replace=False
        #         )
        # else:
        #     selected_frame_indexes = np.random.choice(
        #         len_frames, select_k, replace=True
        #     )
        # selected_frame_indexes.sort()

        # need to set context_num_frames and tarvis_context_num_frames
        # because sometimes a video could be shorter than select_k or select_k_tarvis
        (
            selected_frame_indexes,
        ...
        if self.use_fast:
            ...
            for selected_frame_index in sampled_fast_frame_idxs:
                ...
                fast_video_frames.append(
                    os.path.join(data_dict[0]["video"], frame_id + ".jpg")
                )
        else:
        ...
        for selected_frame_index in tarvis_selected_frame_indexes:
            frame_id = data_dict[0]["frames"][selected_frame_index]
            tarvis_images.append(os.path.join(data_dict[0]["video"], frame_id + ".jpg"))
        ...
        if self.use_fast:
            ...
            text_dict = self.prepare_text(
                context_num_frames, expressions, num_image_tokens=self.patch_token
            )
        ...
        for object_info in data_dict:
            ...
            for anno_id in anno_ids:
                ...
                frames_masks_ = []
                for frame_idx in tarvis_selected_frame_indexes:
    ...
    def prepare_text(
        self, n_frames, expressions, num_image_tokens=256, n_fast_images=50
    ):
        if self.use_fast and not self.fast_token_after_question:
            fast_frame_token_str = (
                f"{self.FAST_IMG_START_TOKEN}"
        ...
        for i, exp in enumerate(expressions):
            ...
            answers.append(random.choice(ANSWER_LIST).replace("[SEG]", "[SEG]" * self.num_tokens_per_expression))
        qa_list = []
        for i, (question, answer) in enumerate(zip(questions, answers)):
            if i == 0:
                ...
                qa_list.append(
                    {
                        "from": "human",
                        "value": frame_tokens + question + after_question_str,
                    }
                )
            else:
                qa_list.append(
    ...
    def __getitem__(self, index):
        ...
        if len(video_objects_infos) > self.select_number:
            selected_indexes = np.random.choice(
                len(video_objects_infos), self.select_number
            )
            video_objects_infos = [
                video_objects_infos[_idx] for _idx in selected_indexes
            ]
        else:
            selected_indexes = np.random.choice(
                len(video_objects_infos), self.select_number, replace=True
            )
            video_objects_infos = [
                video_objects_infos[_idx] for _idx in selected_indexes
            ]
        ...
        data_dict = self.dataset_map_fn(
            video_objects_infos, select_k=self.sampled_frames, select_k_tarvis=self.tarvis_sampled_frames
        )
        ...
        if data_dict.get("images", None) is not None:
            ...
            frames_files = [
                os.path.join(self.image_folder, frame_file)
                for frame_file in frames_files
            ]
            tarvis_frames_files = data_dict["tarvis_images"]
            tarvis_frames_files = [
                os.path.join(self.image_folder, frame_file)
                for frame_file in tarvis_frames_files
            ]
            ...
            for frame_path in tarvis_frames_files:
                frame_image = Image.open(frame_path).convert("RGB")
            ...
            if self.preprocessor is not None:
                if self.arch_type == "qwen":
                    ...
                    num_frame_tokens = int(
                        _data_dict["image_grid_thw"][0].prod()
            ...
            masks = self.decode_mask(
                data_dict["video_masks"], image_size=(ori_height, ori_width)
            )
            if masks is None:
        ...
        if self.use_fast:
            ...
            # process and get masks
            masks = self.decode_mask(
                data_dict["fast_video_masks"], image_size=(ori_height, ori_width)
            )

            if masks is None:
                return self.__getitem__(random.randint(0, self.real_len()))

            data_dict["fast_exists"] = (
                masks.to(dtype=torch.int)
                .sum(dim=(-2, -1))
                .ge(self.exist_thr)
                .unsqueeze(-1)
            )
    ...
    def visualization_debug(self, data_dict):
        save_folder = os.path.join(
            self.save_folder, "sample_{}".format(self.cur_number)
        )
        ...
        for i_image, image_pixel_value in enumerate(pixel_values):
            ...
            cv2.imwrite(
                os.path.join(save_folder_image, "{}.jpg".format(i_image)),
                image_pixel_value,
            )

        # text
        input_text = self.tokenizer.decode(
            data_dict["input_ids"], skip_special_tokens=False
        )
        ...
        for i_obj, obj_masks in enumerate(masks):
            save_folder_mask_obj_folder = os.path.join(
                save_folder_mask, "obj_{}".format(i_obj)
            )
            ...
            for i_frame, f_mask in enumerate(obj_masks):
                ...
                cv2.imwrite(
                    os.path.join(save_folder_mask_obj_folder, "{}.png".format(i_frame)),
                    f_mask,
                )
        return
