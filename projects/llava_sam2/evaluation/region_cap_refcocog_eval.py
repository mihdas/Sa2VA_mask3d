...
def main():
    ...
    for idx in tqdm.tqdm(per_rank_ids):
        ...
        result_dict = {
            "image_id": data_batch["img_id"],
            "image_file": data_batch["image_file"],
        }
        del data_batch["img_id"], data_batch["image_file"]

        prediction = model.predict_forward(**data_batch, tokenizer=tokenizer)[
            "prediction"
        ]
        ...
        text_output = (
            ...
            .replace("region1", "")
            .replace("Region1", "")
            .replace("The region marked by", "")
            .replace("The region marked as", "")
            .replace("The region marked", "")
            .replace("is", "")
            .replace("shows", "")
            .replace(":", "")
            .replace("   ", " ")
