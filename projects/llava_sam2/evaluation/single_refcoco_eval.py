...
def save_mask_overlay(vis_dir, img_id, text_idx, pil_img, mask_bool, gt_masks, rank=0):
    mask_bool = mask_bool.squeeze(0)
    print(mask_bool.shape)
    mask = (mask_bool.astype(np.uint8) * 120)
    print(mask.shape)
    print(gt_masks.squeeze(0).numpy().astype(np.uint8)[0].shape)
    H,W = mask.shape
    ...
    red.putalpha(Image.fromarray(mask))

    over = Image.alpha_composite(base, red).convert("RGB")
    over.save(os.path.join(
        vis_dir, f"{img_id}_t{text_idx}_r{rank}_overlay.png"
    ))

    Image.fromarray((mask_bool.astype(np.uint8) * 255)).save(
        os.path.join(vis_dir, f"{img_id}_t{text_idx}_r{rank}_mask.png")
    )

    Image.fromarray((gt_masks.squeeze(0).numpy().astype(np.uint8)[0] * 255)).save(
        os.path.join(vis_dir, f"{img_id}_t{text_idx}_r{rank}_gtmask.png")
    )
...
def _get_pi3():
    ...
    spatial_model = Pi3().to("cuda").eval()
    from safetensors.torch import load_file
    weight = load_file("/home/burdorf/Downloads/model.safetensors")
    spatial_model.load_state_dict(weight)
...
def parse_args():
    ...
    parser.add_argument(
        "--vis_dir", type=str, default=None,
        help="mask_visualization dir",
    )
...
def main():
    ...
    '''
    spatial_model = _get_pi3()
    spatial_model.forward()
                ...
                for i, text in enumerate(texts):
                    ...
                    if len(pred_mask) == 0:
                        ...
                        if args.vis_dir is not None:
                            save_mask_overlay(args.vis_dir, img_id, i, image, mask_np, gt_masks, rank=rank)
