...
class RESDataset:
    ...
    def __getitem__(self, index):
        ...
        if data_dict.get('image', None) is not None:
            ...
            if "barecat" in self.image_folder:
                barecat_dir = Path(self.image_folder).parent
                barecat_file = barecat_dir / "coco.barecat"
                with barecat.Barecat(barecat_file, readonly=True) as bc:
                    if "COCO" in image_file:
                        _, coco_folder, _ = image_file.split("_")
                        image_file = str(Path(coco_folder) / image_file)
                    with bc.open(Path(image_file), 'rb') as f:
                        image = Image.open(f).convert('RGB')
            else:
                # Fallback to regular file loading if barecat file not found
