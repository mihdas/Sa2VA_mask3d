...
class Sa2VAChatModel(PreTrainedModel):
    ...
    def __init__(self, config: Sa2VAChatConfig, vision_model=None, language_model=None, use_flash_attn=True):
        ...
        self.upscale = nn.Sequential(
            *[ScaleBlock(256) for _ in range(num_upscale)],
        )
    ...
    def extract_3d_feature(self, pixel_values):
        ...
        imgs_5d = pixel_values.unsqueeze(1)
        print(imgs_5d.shape)
        N=1

        #x = (imgs_5d - self.spatial_model.image_mean.cuda()) / self.spatial_model.image_std.cuda()

        BN = B*N
        imgs= imgs_5d.view(BN, C, H, W).to(torch.float32).cuda()
        print(imgs.shape)

        with torch.cuda.amp.autocast(enabled=False):
            #from pudb import set_trace; set_trace()
            hidden = self.spatial_model.encoder(imgs, is_training=True)
        ...
        return embeds_3d.view(B, 5+(H//14)*(W//14), -1)
    ...
    def _llm_forward(
        ...
        x = self._get_pi3()
        with torch.no_grad():
            n_img, _, H, W = pixel_values.shape
            embeds_3d = self.extract_3d_feature(pixel_values)
        embeds_3d = embeds_3d[:,5:,:]

        
        T2D = vit_embeds.size(1)
        ph = pixel_values.shape[-2] //14
        pw = pixel_values.shape[-1] //14
        h2 = w2 = int(T2D**0.5)

        g3 = embeds_3d.view(-1, ph, pw, 2048).permute(0,3,1,2)
        g3 = F.interpolate(g3, size=(h2,w2), mode="bilinear", align_corners=False)
        p3 = g3.permute(0,2,3,1).reshape(-1, h2*w2, 2048)

        if not hasattr(self, "pi3_proj"):
            self.pi3_proj = nn.Linear(2048, vit_embeds.size(-1), bias=False).to(vit_embeds.device)
            for p in self.pi3_proj.parameters():
                p.requires_grad_(True)
        pi3_llm=self.pi3_proj(p3).to(vit_embeds.dtype)

        vit_embeds = vit_embeds + pi3_llm
    ...
    def predict_forward(
        ...
        g_pixel_values = hidden_states[0][-1][mm_inputs["input_ids"] == 151667]
        ...
        #     masks = prob_M[:H,:W]
        masks = F.interpolate(masks.unsqueeze(1), size=(H, W), mode='bilinear', align_corners=False)
