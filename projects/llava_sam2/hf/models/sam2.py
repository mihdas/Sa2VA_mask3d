...
def load_state_dict_to_model(model, state_dict,  logger='current'):
    name_map = {".gamma": ".g_weight"}

    # https://github.com/huggingface/pytorch-image-models/issues/2324
    # For tarvis vit
    all_state_dict_new = {}
    for key in state_dict.keys():
        ...
        all_state_dict_new[new_key] = state_dict[key]
    missing_keys, unexpected_keys = model.load_state_dict(all_state_dict_new)
    if missing_keys:
        print(missing_keys)
        raise RuntimeError()
    if unexpected_keys:
...
class SAM2(nn.Module):
    ...
    def language_embd_inference(self, sam_states, language_embd, nf_nobj=None):
        nf_nobj = None
        high_res_features = [
    ...
    def get_sam2_embeddings_training_like(self, images, expand_size=1):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            feats = self.sam2_model.forward_image(images.cuda(), use_modified=True)
    ...
    def sa2va_language_embd_inference(self, inference_state, language_embd, sample_idx):
        # Regular Way
        num_obj = len(language_embd[0])
        mask_out = []
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            # for frame_idx in range(num_frame):
            for token_idx, frame_idx in enumerate(sample_idx):
                frame_mask_out = []

                for obj_idx in range(num_obj):
                    _language_embd = language_embd[token_idx][obj_idx][None][None]
...
class SAM2Base_(torch.nn.Module):
    ...
    def _build_sam_heads(self):
        ...
        # self.sam_mask_decoder_modified = MaskDecoder(
        #     num_multimask_outputs=3,
        #     transformer=TwoWayTransformer(
        #         depth=2,
        #         embedding_dim=self.sam_prompt_embed_dim,
    ...
    def forward_image(self, img_batch: torch.Tensor, use_modified=False):
        ...
        if self.use_high_res_features_in_sam:
            if use_modified:
                ...
                backbone_out["backbone_fpn"][0] = self.sam_mask_decoder_modified.conv_s0(
                    backbone_out["backbone_fpn"][0]
                )
                backbone_out["backbone_fpn"][1] = self.sam_mask_decoder_modified.conv_s1(
...
class SAM2Base(SAM2Base_):
    ...
    def _forward_sam_heads(
        ...
        - obj_ptr: [B, C] shape, the object pointer vector for the output mask, extracted
          based on the output token from the SAM mask decoder.
        """
        if (not use_modified) or (language_embd is None):
        ...
        if use_modified:
            (
                ...
                sam_output_tokens,
                object_score_logits,
            ) = self.sam_mask_decoder_modified(
                image_embeddings=backbone_features,
                image_pe=self.sam_prompt_encoder.get_dense_pe(),
                sparse_prompt_embeddings=sparse_embeddings,
                dense_prompt_embeddings=dense_embeddings,
