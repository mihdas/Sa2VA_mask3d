...
# from projects.llava_sam2.models.mask2former.simple_tarvis import Tarvis, TarvisV2, TarvisV3
from projects.llava_sam2.models.preprocess.image_resize import DirectResize, ResizeLongestSide, AspectRatioPatchResize
...
language_data_path = "/work/nekrasov/data/language-data"
...
llava_dataset_path = f"{language_data_path}/llava_dataset/llava_data"

# Model
# path = "/p/scratch/llmvidseg/alexey/saved/sam2/InternVL2_5-4B"
path = "/fastwork/nekrasov/saved/InternVL3-1B"
pretrained_pth = None
...
num_tokens_per_expression = 2

# Save
save_steps = 1000
save_total_limit = 2  # Maximum checkpoints to keep (-1 means unlimited)

special_tokens = ["[SEG]", "<p>", "</p>", "<vp>", "</vp>", "<box>", "</box>", "<point>", "</point>"]
...
model = dict(
    ...
    pi3_path="/fastwork/nekrasov/saved/pi3/model.safetensors",
...
video_refytvos_expression_file = (
    f"{refytvos_path}/meta_expressions/train/meta_expressions.json"
)
...
################### Video chat
video_chatunivi_image_folder = f"{video_chatunivi_path}/Activity_Videos_barecat/"
...
################## ViCaS dataset
vicas_expression_file = "/globaldata/ViCaS/vicas_barecat/vicas.barecat"

vicas_multiturn_dataset= dict(
    type=VideoViCaSMultiTurnDataset,
    image_folder="",
    expression_file=vicas_expression_file,
    mask_file=vicas_expression_file,
    tokenizer=tokenizer,
    template_map_fn=dict(type=template_map_fn_factory, template=prompt_template),
    max_length=max_length,
    lazy=True,
...
vicas_gcg_dataset = dict(
    type=VideoViCaSDataset,
    image_folder="/globaldata/ViCaS/vicas_barecat",
    expression_file=vicas_expression_file,
    mask_file=vicas_expression_file,
    tokenizer=tokenizer,
    template_map_fn=dict(type=template_map_fn_factory, template=prompt_template),
...
sa1b_dataset = dict(
    type=SA1BDataset,
    image_folder="/globaldata/sa-1b/sa1b_1M_barecat/sa1b_1M.barecat",
    tokenizer=tokenizer,
    template_map_fn=dict(type=template_map_fn_factory, template=prompt_template),
    max_length=max_length,
    lazy=True,
...
llava_vqa_dataset = dict(
    ...
    special_tokens=special_tokens,
    image_folder=f"{llava_dataset_path}/llava_images_barecat/",
)
...
refcocog_ann_file = (
    f"{glamm_data_root}/GranDf/annotations/train/RefCOCOg_GCG_train.json"
)

grandf_image_path = f"{glamm_data_root}/GranDf_HA_images_barecat/"
grandf_ann_file = f"{glamm_data_root}/GranDf/annotations/train/GranDf_HA_GCG_train.json"

flickr_image_path = f"{glamm_data_root}/flickr30k/flickr30k-images_barecat/"
flickr_ann_file = (
    f"{glamm_data_root}/GranDf/annotations/train/flickr_mergedGT_GCG_train.json"
)
...
# sam2 data
# data_sam2_folder = f'segmentation_datasets/sam_v_full/'
data_sam2_folder = "/globaldata/sa-v"
data_sam2_expression_file = (
    "/globaldata/sa-v/Ref-SAV.json"
)
