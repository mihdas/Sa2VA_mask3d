...
EXPERIMENT_NAME=sa2va_4b_lila_8f_8t
export EXPERIMENT_NOTES=sa2va_4b_lila_8f_8t
...
export PYTHONPATH="${CODE_FOLDER}${PYTHONPATH:+:$PYTHONPATH}"
...
if [ ! -d "$HF_SAVE_PATH" ]; then
    ...
    HF_JOB_ID=$(sbatch --chdir="$CODE_FOLDER" \
          ...
          --export=ALL,PROGRAM="$HF_CONVERSION_COMMAND" \
           "$HOME/github/vision-utils/scripts/juelich_run_v2.bash" | awk '{print $4}')
...
for i in "${!EVAL_COMMANDS[@]}"; do
  ...
  sbatch --chdir="$CODE_FOLDER" \
         ...
         --export=ALL,PROGRAM="$EVAL_COMMAND" \
         "$HOME/github/vision-utils/scripts/juelich_run_v2.bash"
